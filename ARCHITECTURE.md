...
Ansible Galaxy collection `dellemc.unity` (v2.1.0) for Dell Unity storage arrays. Provides 18 modules for declarative, idempotent resource management via the Unity REST API.

Published to Ansible Galaxy under the `dellemc` namespace. Uses the `storops` Python SDK (>=1.2.12).
...
| Python deps | `requirements.txt` | `storops>=1.2.12` |
...
**THEN** the module (1) validates parameters, (2) initialises SDK client via direct `utils.get_unity_connection()` call, (3) fetches current resource state via GET, (4) compares current vs desired, (5) applies no change if identical (`changed=false`), (6) applies delta via SDK PUT/POST if different, (7) returns `changed=true` + updated resource details
...
**GIVEN** a playbook uses `module_defaults` with `group/dellemc.unity.all`
...
| Depends On | For |
|------------|-----|
| `storops` >=1.2.12 | Platform Python SDK |
